...
import org.opennomina.getset.Autoprest;
...
import org.primefaces.model.SortOrder;
	...
	@ManagedBean
	@ViewScoped
	public class Autos04 extends Utils implements Serializable {
		
		private LazyDataModel<Autoprest> lazyModel;  
		
		
		/**
		 * @return the lazyModel
		 */
		public LazyDataModel<Autoprest> getLazyModel() {
			return lazyModel;
		}
	    ...
	    public void init(){
			...
			//Ejecuta el encabezado
	  		...
	  		selectTotales();
	  		//
	  		lazyModel  = new LazyDataModel<Autoprest>(){
				/**
				 * 
				 */
	            ...
	            @Override
				public List<Autoprest> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String,Object> filters) { 
	            	//Filtro
	            	if (filters != null) {
	...
	private String anticipos = "0";
	private String disponible = "0";
	...
	private List<Autoprest> list = new ArrayList<Autoprest>();//LIstar paises
	 ...
	 */
	public void setList(List<Autoprest> list) {
		this.list = list;
	}
	  ...
	  // It returns true if File or directory exists
	     ...
	     **/ 	
	  	public void select(int first, int pageSize, String sortField, Object filterValue) {
	        ...
	        con = ds.getConnection();//Reconoce la base de datos de conección para ejecutar el query correspondiente a cada uno
	        ...
	        case "Oracle":
	        	//Consulta paginada
	        	 ...
	        	 query += " ( select query.*, rownum as rn from";
		   		 query += " ( SELECT anio, mes, to_char(fecha,'dd/mm/yyyy'), codcto, descto, cant, to_char(monto,'999,999,999.99')";
		         query += " FROM autos04";
	        ...
	        while (r.next()){
	        	Autoprest select = new Autoprest();
	            ...
	            select.setVcant(r.getString(6));
	            select.setVmonto(r.getString(7));
	        	//Agrega la lista
	        	list.add(select);
	        	registros = list.size();
	        }
	     ...
	     **/ 	
	     ...
	     try {	
            ...
            String query = null;
	   		...
	   		switch ( productName ) {
	        case "Oracle":
	        	query = "SELECT count_autos04(" + Integer.parseInt(grupo) + ",'" + anio + "','" + mes + "','"  + cedula + "','" + pcodcia + "','" + ((String) filterValue).toUpperCase() + "') from dual";
	             break;
	        case "PostgreSQL":
	        	query = "SELECT count_autos04(" + Integer.parseInt(grupo) + ",'" + anio + "','" + mes + "','"  + cedula + "','" + pcodcia + "','" + ((String) filterValue).toUpperCase() + "')";
	             break;
	  		}
	     ...
	     **/ 	
	  	private void selectEncab(String cedula, String paramcia, String opc)  {
	  	    ...
	  	    //Busca la consulta para encabezado básico
	  		       ...
	  		       query += " from autos04";
	  		       query += " where CAST(anio AS text) like (select cast(max(anio) as text) from autos04 where cedula like'" + cedula +"%')";
	  		       query += " and CAST(mes AS text) like (select cast(max(mes) as text) from autos04 where cedula like'" + cedula +"%' and CAST(anio AS text) like (select cast(max(anio) as text) from autos04 where cedula like'" + cedula +"%'))";
	  	  ...
	  	  String queryora = "";
	  	    ...
	  	    //Busca la consulta para encabezado básico
	  	    	...
	  	    	queryora += " from autos04";
	  	    	queryora += " where anio like (select max(anio) from autos04 where cedula like'" + cedula +"%')";
	  	    	queryora += " and mes like (select max(mes) from autos04 where cedula like'" + cedula +"%' and anio like (select max(anio) from autos04 where cedula like'" + cedula +"%'))";
	     ...
	     **/ 
	  	...
	  	//Busca los registros
	  		try {     		
	  			...
	  			String query = "select to_char(sum(a.cant),'999,999,999.99'), to_char(sum(a.monto),'999,999,999.99')"
		                 + ", to_char(subquery1.anticipos,'999,999,999.99') , to_char(((sum(a.monto) * .75) - subquery1.anticipos),'999,999,999.99'), a.cedula"
		                 + " from autos04 a," 
		                 + " (select  cedula, anticipos, disponible"
		                 + " from autos04"
		                 + " where trim(pcodcia) = '" + pcodcia + "' and trim(cedula) like '" + cedula + "%'"
		                 + " and anio = (select max(anio) from autos04 where trim(pcodcia) = '" + pcodcia + "' and trim(cedula) like '" + cedula + "%')"
		                 + " and mes = (select max(mes)" 
		                            + " from autos04" 
		                            + " where trim(pcodcia) = '" + pcodcia + "' and trim(cedula) like '" + cedula + "%'"
		                            + " and anio = (select max(anio)" 
		                            + " from autos04 where trim(pcodcia) = '" +pcodcia + "' and trim(cedula) like '" +cedula + "%')) group by cedula, anticipos, disponible) subquery1"
		                 + " where a.cedula = subquery1.cedula "
		                 + " and trim(a.pcodcia) = '" +pcodcia + "' and trim(a.cedula) like '" +cedula + "%' and grupo = '" + grupo + "'"
		                 + " group by a.cedula, subquery1.cedula, subquery1.anticipos, subquery1.disponible";
