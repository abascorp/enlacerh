...
import org.opennomina.getset.Autoprestamo;
...
import org.primefaces.model.SortOrder;
	 ...
	 */
	/**
	 * Clase para leer, insertar, modificar y eliminar registros sede la tabla
	 * autos01 registro de usuarios de autoservicio
	 *  */
	@ManagedBean
	@ViewScoped
	public class Autos06 extends Utils implements Serializable {
	 ...
	 */
	public void setLazyModel(LazyDataModel<Autoprestamo> lazyModel) {
	     ...
	     **/ 	
	     ...
	     try {	
	    	Context initContext = new InitialContext();     
	 		...
	 		switch ( productName ) {
	        case "Oracle":
	        	query = "SELECT count_autos06(" + Integer.parseInt(grupo) + ",'" + anio + "','" + mes + "','"  + cedula + "','" + pcodcia + "','" + ((String) filterValue).toUpperCase() + "') from dual";
	             break;
	        case "PostgreSQL":
	        	query = "SELECT count_autos06(" + Integer.parseInt(grupo) + ",'" + anio + "','" + mes + "','"  + cedula + "','" + pcodcia + "','" + ((String) filterValue).toUpperCase() + "')";
	             break;
	  		}
	 ...
	 /**
		...
		public int getRegistros() {
			...
			**/ 	
			...
			query += " from autos06";
			query += " where CAST(anio AS text) like (select cast(max(anio) as text) from autos06 where cedula like'" + cedula +"%')";
			query += " and CAST(mes AS text) like (select cast(max(mes) as text) from autos06 where cedula like'" + cedula +"%' and CAST(anio AS text) like (select cast(max(anio) as text) from autos06 where cedula like'" + cedula +"%'))";
			...
			//Busca la consulta para encabezado básico
				...
				queryora += " from autos06";
				queryora += " where anio like (select max(anio) from autos06 where cedula like'" + cedula +"%')";
				queryora += " and mes like (select max(mes) from autos06 where cedula like'" + cedula +"%' and anio like (select max(anio) from autos06 where cedula like'" + cedula +"%'))";
