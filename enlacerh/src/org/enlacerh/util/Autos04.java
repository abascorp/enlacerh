...
import org.enlacerh.getset.Autoprest;
	     ...
	     **/ 
	  	...
	  	//Busca los registros
	  		try {     		
	  			...
	  			String query = "SELECT "
			  			  	 + "TRIM(to_char(SUM(NMM036.CANTID),'999,999,999.99')) TOTCAN, "
			  				 + "TRIM(to_char(SUM(NMM036.MONABO),'999,999,999.99')) TOTABO, " 
	  			             + "TRIM(to_char(OPENBIZVIEW.REP_RRHH_SALDO(NMM036.CIA_CODCIA,NMM036.TNOM_TIPNOM,NMM036.TRAB_FICTRA,5205,SYSDATE),'999,999,999.99')) TOTANT, "
	  			             + "TRIM(to_char((SUM(NMM036.MONABO) * .75) - OPENBIZVIEW.REP_RRHH_SALDO(NMM036.CIA_CODCIA,NMM036.TNOM_TIPNOM,NMM036.TRAB_FICTRA,5205,SYSDATE),'999,999,999.99')) TOTDIS "
	  			             + "FROM "
	  			             + "NMM036 NMM036 LEFT OUTER JOIN INFOCENT.NMM001 NMM001 ON NMM036.CIA_CODCIA = NMM001.CIA_CODCIA AND "
	  			                                                                     + "NMM036.TNOM_TIPNOM = NMM001.TNOM_TIPNOM AND "
	  			                                                                   + "NMM036.TRAB_FICTRA = NMM001.FICTRA "
	  			             + "WHERE "
	  			             + "NMM036.CIA_CODCIA = '" + pcodcia + "' AND "
	  			             + "TRIM(NMM001.CEDULA) = '" + cedula + "' "
	  			             + "GROUP BY "
	  			             + "NMM036.CIA_CODCIA,NMM036.TNOM_TIPNOM,NMM036.TRAB_FICTRA ";
