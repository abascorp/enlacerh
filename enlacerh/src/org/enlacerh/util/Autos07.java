...
import org.enlacerh.getset.Autoprestamo;
...
import org.primefaces.model.SortOrder;
	...
	@ManagedBean
	@ViewScoped
	public class Autos07 extends Utils implements Serializable {
		
       private LazyDataModel<Autoprestamo> lazyModel;  
       HttpServletRequest rq = (HttpServletRequest) FacesContext.getCurrentInstance().getExternalContext().getRequest();
		 ...
		 */
		public LazyDataModel<Autoprestamo> getLazyModel() {
			return lazyModel;
		}
	    ...
	    public void init(){
			...
			//Ejecuta el encabezado
	  		...
	  		//
	  		lazyModel  = new LazyDataModel<Autoprestamo>(){
				/**
				 * 
				 */
	            ...
	            @Override
				public List<Autoprestamo> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String,Object> filters) { 
	            	//Filtro
	            	if (filters != null) {
	...
	private String utidis = "0";
	private String fecant = "0";
	private String saldot = "0";
	private Object filterValue = "";
	private List<Autoprestamo> list = new ArrayList<Autoprestamo>();//LIstar paises
	 ...
	 * @return the list
	 */
	public List<Autoprestamo> getList() {
		return list;
	}
	 ...
	 */
	public void setList(List<Autoprestamo> list) {
		this.list = list;
	}
	...
	ResultSet r;
	      ...
	      //Reconoce la base de datos de conección para ejecutar el query correspondiente a cada uno
	        ...
	        case "Oracle":
	        	...
	        	query += " ( select query.*, rownum as rn from";
		  		query += " ( SELECT anio, mes, codcto, descto";
		        query += " , to_char(monto,'999,999,999.99'), to_char(fn_sdouti(pcodcia, cedula, mes, anio),'999,999,999.99')";
		        query += " FROM autos07";
		        query += " where TO_CHAR(ANIO) || LPAD(TO_CHAR(MES),2,'00') >= FN_INIUTI(PCODCIA,PTIPNOM,TO_CHAR(SYSDATE,'YYYY'),TO_CHAR(SYSDATE,'MM'))";
		        ...
		        query += " order by anio desc, mes desc) query";
	        ...
	        case "PostgreSQL":
	        	//Consulta paginada
		  		query = " SELECT anio, mes, codcto, descto";
		        query += " , to_char(monto,'999,999,999.99'), to_char(saldo,'999,999,999.99')";
		        query += " FROM autos07";
	     ...
	     **/ 	
	     ...
	     try {	
	    	Context initContext = new InitialContext();     
	 		...
	 		switch ( productName ) {
	        case "Oracle":
	        	query = "SELECT count_autos07(" + Integer.parseInt(grupo) + ",'" + anio + "','" + mes + "','"  + cedula + "','" + pcodcia + "','" + ((String) filterValue).toUpperCase() + "') from dual";
	             break;
	        case "PostgreSQL":
	        	query = "SELECT count_autos07(" + Integer.parseInt(grupo) + ",'" + anio + "','" + mes + "','"  + cedula + "','" + pcodcia + "','" + ((String) filterValue).toUpperCase() + "')";
	             break;
	  		}
	 ...
	 /**
		...
		**/ 	
		...
		query += " from autos07";
		query += " where CAST(anio AS text) like (select cast(max(anio) as text) from autos07 where cedula like'" + cedula +"%')";
		query += " and CAST(mes AS text) like (select cast(max(mes) as text) from autos07 where cedula like'" + cedula +"%' and CAST(anio AS text) like (select cast(max(anio) as text) from autos07 where cedula like'" + cedula +"%'))";
		...
		//Busca la consulta para encabezado básico
			...
			queryora += " from autos07";
			queryora += " where anio like (select max(anio) from autos07 where cedula like'" + cedula +"%')";
			queryora += " and mes like (select max(mes) from autos07 where cedula like'" + cedula +"%' and anio like (select max(anio) from autos07 where cedula like'" + cedula +"%'))";
	  	...
	  	//Busca los registros
	  		try {     		
	  			//Consulta los totales acumulados
	  			  ...
	  			  consulta.selectPntGenerica("SELECT " +
											 " SDO.SDOUTI, " +
											 " TO_CHAR(ANT.FECMOV,'DD/MM/YYYY') FECMOV, " +
											 " TRIM(TO_CHAR(ANT.SALDOT,'999,999,999.99')) SALDOT " +
											 " FROM (SELECT  " +
											 "       TRIM(TO_CHAR(FN_SDOUTI('" + pcodcia + "','" + cedula + "%',TO_CHAR(SYSDATE,'MM'),TO_CHAR(SYSDATE,'YYYY')),'999,999,999.99')) SDOUTI FROM DUAL) SDO " +
											 " LEFT OUTER JOIN " + 
											 " (SELECT " +
											 "  A.CIA_CODCIA, A.TNOM_TIPNOM, A.TRAB_FICTRA, A.FECMOV, A.SALDOT " +
											 "  FROM NMM013 A LEFT OUTER JOIN NMM001 B ON A.CIA_CODCIA = B.CIA_CODCIA AND " +
											 "                                            A.TNOM_TIPNOM = B.TNOM_TIPNOM AND " +
											 "                                            A.TRAB_FICTRA = B.FICTRA " +
											 "  WHERE " +
											 "  A.CIA_CODCIA = '" + pcodcia + "' AND " +
											 "  TRIM(B.CEDULA) = '" + cedula + "' AND " + 
											 "  A.CTO_CODCTO = 5300 AND " + 
											 "  A.FECMOV IN (SELECT " +
											 "               MAX(B.FECMOV) " + 
											 "               FROM " + 
											 "               NMM013 B " +
											 "               WHERE " + 
											 "               B.CIA_CODCIA = A.CIA_CODCIA AND " +
											 "               B.TNOM_TIPNOM = A.TNOM_TIPNOM AND " +
											 "               B.TRAB_FICTRA = A.TRAB_FICTRA AND " +
											 "               B.CTO_CODCTO = A.CTO_CODCTO AND " +
											 "               B.FECMOV <= SYSDATE)) ANT ON 1=1", JNDI);	  			 	  			
